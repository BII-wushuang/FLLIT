function [samples_idx] = get_2Dsamples_idx_fn(params,good_sampling_idx,fn,tot_idx)

% Extract the sampling positions (2D) and their labels given the file names
...
for i_img = 1:imgs_no
    ...
    img = load_gt_img(fn{i_img},params);
    ...
    [tmp_pos(pos_collected_no+1:pos_collected_no+ns_pos,2),...
        tmp_pos(pos_collected_no+1:pos_collected_no+ns_pos,3)] = ind2sub(size(img(:,:,1)),...
        good_sampling_idx{i_img}.pos);
    
    [tmp_neg(neg_collected_no+1:neg_collected_no+ns_neg,2),...
        tmp_neg(neg_collected_no+1:neg_collected_no+ns_neg,3)] = ind2sub(size(img(:,:,1)),...
