function [samples_idx] = get_samples_idx_3d(params,good_sampling_idx,fn,tot_idx)

% Extract the sampling positions (3D) and their labels
%
...
tmp_pos = zeros(tot_idx.pos,4);
tmp_neg = zeros(tot_idx.neg,4);
pos_collected_no = 0;
neg_collected_no = 0;
...
for i_img = 1:imgs_no
    ...
    img = import_3D_data_v4(fn{i_img});
    ...
    [tmp_pos(pos_collected_no+1:pos_collected_no+ns_pos,2),...
        tmp_pos(pos_collected_no+1:pos_collected_no+ns_pos,3),...
        tmp_pos(pos_collected_no+1:pos_collected_no+ns_pos,4)] = ind2sub(size(img),...
        good_sampling_idx{i_img}.pos);
    
    [tmp_neg(neg_collected_no+1:neg_collected_no+ns_neg,2),...
        tmp_neg(neg_collected_no+1:neg_collected_no+ns_neg,3),...
        tmp_neg(pos_collected_no+1:pos_collected_no+ns_pos,4)] = ind2sub(size(img),...
        good_sampling_idx{i_img}.neg);            
