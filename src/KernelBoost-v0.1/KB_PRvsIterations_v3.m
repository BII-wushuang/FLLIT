function [F,Pre,Rec] = KB_PRvsIterations_v3(params,data,weak_learners)
...
for i_w = 1 : wl_no
    ...
    for i_img = 1 : params.gts.test.imgs_no
        ...
        img = data.test.imgs.X(i_img,1:3);
        
        img_s = zeros(size(img{1}));
        
        for i_b =1 : length(img)
...
%         img_s = sum(img,3);
        ...
        img{4} = lap_gaussian_F(img_s);
        
        for i_b = 1 : 3
            
            img{i_b + 4} =  data.test.diff_imgs.X{i_img,i_b} * 10;
            
        end
        
        
        new_score = KB_evaluate_v2(img,wl,params);
