...
for j = 1:nlegs
    ...
    pathpca(j,:,:) = pca([nx(startframe : endframe,j),ny(startframe : endframe,j)]);
    centre(j,:) = [sum(nx(startframe : endframe,j))/nframes, sum(ny(startframe : endframe,j))/nframes];
    ...
    k = -100:100;
    [length_x{j}, length_y{j}] = intersections(centre(j,1)+k*pathpca(j,1,1),centre(j,2)+k*pathpca(j,1,2),nx(convex{j}+startframe-1,j),ny(convex{j}+startframe-1,j));
    domain_length(j) = sqrt((length_x{j}(2)-length_x{j}(1))^2+(length_y{j}(2)-length_y{j}(1))^2);
    [width_x{j}, width_y{j}] = intersections(centre(j,1)+k*pathpca(j,2,1),centre(j,2)+k*pathpca(j,2,2),nx(convex{j}+startframe-1,j),ny(convex{j}+startframe-1,j));
    domain_width(j) = sqrt((width_x{j}(2)-width_x{j}(1))^2+(width_y{j}(2)-width_y{j}(1))^2);
...
for j = 1:nlegs
    ...
    for k = j+1:nlegs
        ...
        if(isempty(intersections(P1.x,P1.y,P2.x,P2.y)))
            ...
            overlap = polyarea(C.x,C.y);
        end
         fprintf(fileID,'%s \t %0.1f \n', ['Overlap ' legs_id{j} '&' legs_id{k}], overlap);
    end
end
fclose(fileID);
